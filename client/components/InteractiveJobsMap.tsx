...
export default function InteractiveJobsMap({
  ...
  const geoJobs = jobs.filter((job) => job.coordinates !== null);
  ...
  const majorCities = [
    ...
    {
      name: "السليمانية",
      coordinates: { lat: 35.565, lng: 45.4297 },
  ...
  const flyToCity = (coordinates: { lat: number; lng: number }) => {
  ...
  const JobMarker = ({
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "relative w-8 h-8 rounded-full shadow-lg border-2 border-white transition-all duration-300",
            job.featured
              ? "bg-gradient-to-r from-purple-500 to-pink-500"
              : job.urgent
                ? "bg-gradient-to-r from-red-500 to-orange-500"
                : "bg-gradient-to-r from-blue-500 to-cyan-500",
            isSelected && "shadow-xl ring-4 ring-white/50",
          )}
        >
        ...
        <div className="bg-white rounded-t-3xl lg:rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              <button
                onClick={() => onJobApply(selectedJob.id)}
      ...
      <div className="relative w-full h-[500px] bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl overflow-hidden border border-gray-200">
        ...
        <div className="absolute inset-0 p-8">
          ...
          >
            {/* حدود العراق المبسطة */}
            <path
              d="M80 50 L320 50 L320 80 L350 100 L350 200 L320 220 L280 250 L200 250 L150 230 L100 200 L80 150 Z"
              fill="currentColor"
        ...
        {majorCities.map((city) => (
          <button
            key={city.name}
            onClick={() => flyToCity(city.coordinates)}
            className="absolute transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-full p-2 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200 group"
            style={{
          ...
          >
            <div className="text-center">
              ...
              <div className="text-xs text-blue-600">{city.jobs} وظيفة</div>
            </div>
          </button>
        ))}
        ...
        {showMapControls && (
          <div className="absolute top-4 right-4 space-y-2">
            ...
            <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-1">
              <button
                onClick={() => setMapCenter({ lat: 33.3152, lng: 44.3661 })}
                className="p-2 hover:bg-gray-100 rounded transition-colors"
                title="العودة للوسط"
              >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        <div className="bg-blue-50 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-blue-600 mb-1">
            {geoJobs.filter((job) => job.featured).length}
          </div>
          <div className="text-sm text-blue-700">وظائف مميزة</div>
        </div>

        <div className="bg-red-50 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-red-600 mb-1">
            {geoJobs.filter((job) => job.urgent).length}
          </div>
          <div className="text-sm text-red-700">وظائف عاجلة</div>
        </div>

        <div className="bg-green-50 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-green-600 mb-1">
            {geoJobs.filter((job) => job.remote).length}
          </div>
          <div className="text-sm text-green-700">عمل عن بُعد</div>
        </div>

        <div className="bg-purple-50 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-purple-600 mb-1">
            {majorCities.filter((city) => city.jobs > 0).length}
          </div>
          <div className="text-sm text-purple-700">مدن نشطة</div>
        </div>
      </div>
