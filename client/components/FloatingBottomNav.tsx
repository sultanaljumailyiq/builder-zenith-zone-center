...
type SectionType = "marketplace" | "community" | "jobs" | "education" | "default";
...
export default function FloatingBottomNav({
  ...
  useEffect(() => {
    ...
    if (path.includes("/favorites") && sectionParam) {
      const validSections: SectionType[] = ["marketplace", "community", "jobs", "education"];
      if (validSections.includes(sectionParam as SectionType)) {
        ...
        path.includes("/brands") || path.includes("/suppliers") ||
        path.includes("/cart") || path.includes("/product/")) {
    ...
    else if (path.includes("/dental-supply") || path.includes("/marketplace")) setActiveTab("marketplace");
    else if (path.includes("/favorites")) setActiveTab("favorites");
    else if (path.includes("/cart")) setActiveTab("cart");
    else if (path.includes("/profile") || path.includes("/dashboard")) setActiveTab("profile");
  ...
  const handleCreatePost = () => {
    ...
    setTimeout(() => {
      const createPostButton = document.querySelector('[data-create-post]') as HTMLElement;
      const createPostInput = document.querySelector('textarea[placeholder*="ما الذي تريد مشاركته"]') as HTMLTextAreaElement;
  ...
  const getSectionName = () => {
    switch (currentSection) {
      case "marketplace": return "المتجر";
      case "community": return "المجتمع";
      case "jobs": return "الوظائف";
      case "education": return "التعليم";
      default: return "ال��طبيق";
    }
  };

  const getSectionColor = () => {
    switch (currentSection) {
      case "marketplace": return "from-blue-500 to-cyan-500";
      case "community": return "from-emerald-500 to-teal-500";
      case "jobs": return "from-violet-500 to-purple-500";
      case "education": return "from-rose-500 to-pink-500";
      default: return "from-gray-500 to-gray-600";
    }
  };

  const mainTabs = getMainTabs();
  const { commonItems, quickActions, settingsItems, sectionItems } = getMoreMenuItems();

  const TabButton = ({ tab, index, isDesktop = false }: { tab: any; index: number; isDesktop?: boolean }) => {
          ...
          >
            ...
            <tab.icon
              className={cn(
                ...
                isSpecial || isActive ? "text-white" : "text-gray-600 group-hover:text-gray-800"
              )}
            />
          </div>
          {isDesktop ? (
            <span
              className={cn(
                ...
                isSpecial || isActive ? "text-transparent bg-clip-text bg-gradient-to-r " + tab.gradient : "text-gray-700"
              )}
            >
              {tab.label}
            </span>
            ...
            <span
              className={cn(
                ...
                isSpecial || isActive ? "text-transparent bg-clip-text bg-gradient-to-r " + tab.gradient : "text-gray-600"
              )}
            >
              {tab.label}
            </span>
      ...
      >
        ...
        {isDesktop ? (
          <span
            className={cn(
              "text-sm font-semibold transition-all duration-300",
              isActive ? "text-transparent bg-clip-text bg-gradient-to-r " + tab.gradient : "text-gray-700 group-hover:text-gray-800"
            )}
          >
            {tab.label}
          </span>
          ...
          <>
            <span
              className={cn(
                "text-xs font-semibold mt-1 transition-all duration-300 sm:text-sm",
                isActive ? "text-transparent bg-clip-text bg-gradient-to-r " + tab.gradient : "text-gray-600 group-hover:text-gray-800"
              )}
            >
              {tab.label}
            </span>
        ...
        <div className="p-4 sm:p-6 max-h-[75vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                أنت حا��ياً في: <span className={`font-semibold bg-gradient-to-r ${getSectionColor()} bg-clip-text text-transparent`}>
                  {getSectionName()}
                </span>
              </p>
            </div>
            <div className={`w-6 h-0.5 sm:w-8 sm:h-1 bg-gradient-to-r ${getSectionColor()} rounded-full`} />
          </div>
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-sm font-semibold text-gray-700">أدوات سريعة</h4>
            </div>
                ...
                <div key={item.label} className="group">
                  {item.path ? (
                    <Link
                      ...
                      onClick={() => setShowMoreMenu(false)}
                    >
                      <div className={`w-12 h-12 bg-${item.color}-100 rounded-2xl flex items-center justify-center mb-2 group-hover:shadow-lg transition-all duration-300`}>
                        <item.icon className={`w-6 h-6 text-${item.color}-600`} />
                      </div>
                      <span className="text-xs font-semibold text-gray-900 text-center">{item.label}</span>
                    </Link>
                  ) : (
                    <button
                      onClick={() => {
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-sm font-semibold text-gray-700">الحساب والملف الشخصي</h4>
            </div>
                ...
                <div key={item.label} className="group">
                  {item.path ? (
                    <Link
                      ...
                      onClick={() => setShowMoreMenu(false)}
                    >
                      <div className={`w-10 h-10 bg-${item.color}-100 rounded-xl flex items-center justify-center group-hover:shadow-md transition-all duration-300`}>
          ...
          {sectionItems.map((group, groupIndex) => (
            <div key={group.title} className="mb-6">
              <h4 className="text-sm font-semibold text-gray-700 mb-3">{group.title}</h4>
              <div className="space-y-2">
                    ...
                    {item.path ? (
                      <Link
                        ...
                        className={cn(
                          ...
                          item.isSection && currentSection === item.path.split('/')[1] && "bg-blue-50 border border-blue-200"
                        )}
                        onClick={() => setShowMoreMenu(false)}
                      >
                        <div className={`w-10 h-10 bg-${item.color}-100 rounded-xl flex items-center justify-center group-hover:shadow-md transition-all duration-300`}>
                        ...
                        {item.isSection && currentSection === item.path.split('/')[1] && (
          ...
          <div className="mt-6 pt-4 border-t border-gray-200">
            <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h4 className="text-sm font-semibold text-gray-900">إعدادات متقدمة</h4>
                  <p className="text-xs text-gray-600">المزيد من خيارات التحكم</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <span className="text-xs font-medium text-gray-900">المظهر</span>
                </button>
                ...
                >
                  ...
                  <span className="text-xs font-medium text-gray-900">عن التطبيق</span>
                </button>
              </div>
            </div>
          </div>
